...
public class PID_ATune
{
  ...
  Tuning factors are determined by the process variable response to particular known perturbation.  Simply,
  hit the process with a known blow and measure how it responds.

  The perturbation used by this tuner is every time the process variable falls below the Setpoint then
  hit the process with a specified step up in control and every time the process variable goes above the Setpoint
  ...
  to be .76 (controller setting of the fraction of the full power available).

  Next a control signal perturbation must be chosen.  This number will be added and subtracted from the
  ...
  as the parameters must have the units converted.
  ...
  The example program shows this.  Better yet, just tune with the native units of the controller.
